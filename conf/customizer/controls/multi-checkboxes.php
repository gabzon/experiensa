...
//class Cstmzr_Category_Checkboxes_Control extends WP_Customize_Control {
    ...
    /*public function render_content() {
        ...
        foreach ( get_categories() as $category ) {
            echo    '<label>
                        <input type="checkbox" name="category-' . $category->term_id . '" id="category-' . $category->term_id . '" class="cstmzr-category-checkbox"> ' . $category->cat_name .
                    '</label>
                    <br>';
        }

        // Loads the hidden input field that stores the saved category list.
        ?><input type="hidden" id="<?= $this->id; ?>" class="cstmzr-hidden-categories" <?= $this->link(); ?> value="<?= sanitize_text_field( $this->value() ); ?>">
        <?php
    }*/
//}